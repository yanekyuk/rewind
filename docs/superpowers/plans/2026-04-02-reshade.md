...
| `rewind-core/src/localconfig.rs` | Create | VDF parser: read/write Steam launch options in localconfig.vdf |
...
## Task 6: Implement localconfig.vdf reader/writer

**Files:**
- Create: `rewind-core/src/localconfig.rs`
- Modify: `rewind-core/src/lib.rs`

- [ ] **Step 1: Write failing tests**

Create `rewind-core/src/localconfig.rs` with the test module only:
...
```
cargo test -p rewind-core read_existing_launch write_replaces write_inserts write_empty
```

Expected: compile error — `extract_launch_options`, `write_launch_options` not defined.

- [ ] **Step 3: Implement the VDF parser and writer**

Add the full implementation to `localconfig.rs` before the `#[cfg(test)]` block:
...
fn extract_launch_options(content: &str, app_id: u32) -> Option<String> {
...
fn set_launch_options(content: &str, app_id: u32, options: &str) -> String {
    let app_key = format!("\"{}\"", app_id);
    let lines: Vec<&str> = content.lines().collect();
...
```
cargo test -p rewind-core read_existing_launch write_replaces write_inserts write_empty read_missing read_unknown
```
...
```bash
git add rewind-core/src/localconfig.rs rewind-core/src/lib.rs
git commit -m "feat: add localconfig.rs for reading/writing Steam launch options in VDF"
```

---

## Task 7: Add app state for ReshadeSetup screen
...
let reshade_launch_hint: String = {
    match entry.and_then(|e| e.reshade.as_ref()).filter(|r| r.enabled) {
        Some(r) if r.original_launch_options.is_some() => {
            // We wrote it automatically — show confirmation instead of the raw command
            "\n  Launch options: written automatically".to_string()
        }
        Some(r) => {
            // localconfig not found — show manual hint as fallback
